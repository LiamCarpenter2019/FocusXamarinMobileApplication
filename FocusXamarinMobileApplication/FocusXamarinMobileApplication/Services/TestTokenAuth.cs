...
internal class TestTokenAuth : IDisposable
{
    ...
    public ClientContext GetContext(Uri web)
    {
        ...
        var UserName = cv2.Username;
        var password = cv2.Password;
        ...
        context.ExecutingWebRequest += (sender, e) =>
        {
            var accessToken = EnsureAccessTokenAsync(new Uri($"{web.Scheme}://{web.DnsSafeHost}"),
                UserName,
                new NetworkCredential(string.Empty, secureString).Password).GetAwaiter().GetResult();
    ...
    private async Task<string> AcquireTokenAsync(Uri resourceUri, string username, string password)
    {
        ...
        var clientId = cv2.defaultAADAppId;
        var body =
